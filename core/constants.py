...
# ── Time controls ─────────────────────────────────────────
# key → (label, base minutes, increment seconds). Every preset is clocked:
# we issue "go wtime/btime winc/binc" so engines manage their own time and
# lose on time when the clock runs out. A base of None means no clock at
# all (fixed think time per move) — no preset uses that any more.
TIME_CONTROLS = {
    "bullet":  ("Bullet (1+0)",   1.0, 0.0),
    "blitz":   ("Blitz (3+2)",    3.0, 2.0),
    "rapid":   ("Rapid (10+5)",  10.0, 5.0),
    "classic": ("Classic (30+0)", 30.0, 0.0),
}

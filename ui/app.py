# ═══════════════════════════════════════════════════════════
#  ui/app.py — Main ChessGUI application class (responsive)
# ═══════════════════════════════════════════════════════════
...
from ui.dialogs import ask_promotion, ask_stop_result, make_search_bar, ask_opening_choice
...
from ui.theme import (
    FONT_FAMILY, FONT_MONO, FONT_HEADING, FONT_BODY, FONT_SMALL,
    FONT_TINY, FONT_MONO_SM, FONT_MONO_XS, SEP_COLOR,
    separator, accent_line,
)
from ui.widgets import label, heading, button, toolbar_button, entry
...
class ChessGUI:
    ...
    LEFT_MIN_W  = 260
    RIGHT_MIN_W = 275
    BOARD_MIN   = 400          # minimum board pixel size (8 * 50)
    ...
    def _draw_eval_bar(self, cp=None):
        ...
        bar_h = self.eval_canvas.winfo_height() or (self.sq_size * 8)
        bar_w = self.eval_canvas.winfo_width()  or 22
        ...
        self.eval_canvas.create_text(
            bar_w // 2, ty, text=txt,
            font=(FONT_MONO, 7, "bold"), fill=fg, anchor="center")
        self.eval_canvas.create_rectangle(
            0, 0, bar_w - 1, bar_h - 1, outline="#555", width=1)
    ...
    def _update_analyzer_lbl(self):
        if not hasattr(self, "analyzer_lbl"):
            return
        if self.analyzer and self.analyzer.alive:
    ...
    def _pick_opening(self):
        ...
        if moves:
            ...
            if hasattr(self, "_tbtn_pick"):
                self._tbtn_pick.config(
                    text=f"📖 {short[:20]}…" if len(short) > 20 else f"📖 {short}",
                    fg="#00BFFF", bg=ACCENT)
    ...
    def _tournament_list(self):
        try:
            from tournament.manager import open_tournament_list
            ...
            messagebox.showinfo("Tournament", "tournament module not found.")

    # ═══════════════════════════════════════════════════════
    #  UI BUILDERS — Responsive grid layout
    # ═══════════════════════════════════════════════════════

    def _build_ui(self):
        """Create the three-panel responsive layout."""

        # Root grid: toolbar row 0 (fixed), main row 1 (expands)
        self.root.columnconfigure(0, weight=0, minsize=self.LEFT_MIN_W)
        self.root.columnconfigure(1, weight=1, minsize=self.BOARD_MIN)
        self.root.columnconfigure(2, weight=0, minsize=self.RIGHT_MIN_W)
        self.root.rowconfigure(0, weight=0)
        self.root.rowconfigure(1, weight=1)

        # ── Top toolbar ───────────────────────────────────
        tb = tk.Frame(self.root, bg="#0F0F1E", height=44)
        tb.grid(row=0, column=0, columnspan=3, sticky="ew")
        tb.grid_propagate(False)
        self._build_toolbar(tb)

        # ── Left panel (scrollable) ───────────────────────
        left_outer = tk.Frame(self.root, bg=PANEL_BG)
        left_outer.grid(row=1, column=0, sticky="nsew", padx=(6, 0), pady=(4, 6))

        # Scrollable canvas for left panel
        self._left_canvas = tk.Canvas(left_outer, bg=PANEL_BG, highlightthickness=0)
        self._left_scroll = tk.Scrollbar(left_outer, orient="vertical",
                                          command=self._left_canvas.yview)
        self._left_inner = tk.Frame(self._left_canvas, bg=PANEL_BG)

        self._left_inner.bind("<Configure>",
            lambda e: self._left_canvas.configure(scrollregion=self._left_canvas.bbox("all")))
        self._left_canvas.create_window((0, 0), window=self._left_inner, anchor="nw")
        ...
        self._left_canvas.bind_all("<MouseWheel>", _on_mousewheel, add="+")

        self._build_left(self._left_inner)

        # ── Center panel ──────────────────────────────────
        cp = tk.Frame(self.root, bg=BG)
        cp.grid(row=1, column=1, sticky="nsew", padx=6, pady=(4, 6))
        cp.columnconfigure(0, weight=1)
        cp.rowconfigure(3, weight=1)   # board row expands
        self._build_center(cp)

        # ── Right panel ───────────────────────────────────
        rp = tk.Frame(self.root, bg=PANEL_BG)
        rp.grid(row=1, column=2, sticky="nsew", padx=(0, 6), pady=(4, 6))
    ...
    def _build_toolbar(self, tb):
        ...
        tk.Label(tb, text="♟  ENGINE ARENA", bg="#0F0F1E", fg=ACCENT,
                 font=(FONT_FAMILY, 12, "bold"), padx=14).pack(side="left")
        ...
        for txt, cmd in [
            ...
            toolbar_button(tb, txt, cmd).pack(side="left", padx=2, pady=5)

        # Opening preset controls (right side)
        ...
        self._tbtn_clear_preset.bind("<Enter>",
            lambda e: self._tbtn_clear_preset.config(bg=BTN_HOV))
        self._tbtn_clear_preset.bind("<Leave>",
            lambda e: self._tbtn_clear_preset.config(bg="#1E1E3A"))
    ...
    def _build_left(self, p):
        tk.Label(p, text="CONFIGURATION", bg=PANEL_BG, fg=ACCENT,
                 font=(FONT_FAMILY, 11, "bold")).pack(pady=(12, 4))
        accent_line(p)

        # Play mode
        mode_frame = tk.Frame(p, bg=PANEL_BG)
        mode_frame.pack(fill="x", padx=10, pady=(8, 4))
        heading(mode_frame, "PLAY MODE:").pack(anchor="w")
        for lbl_text, value in [("Engine vs Engine", "engine_vs_engine"),
                                ("Play vs Engine",   "human_vs_engine")]:
            tk.Radiobutton(
                mode_frame, text=lbl_text, variable=self.play_mode, value=value,
                bg=PANEL_BG, fg=TEXT, selectcolor=BTN_BG,
                activebackground=PANEL_BG, activeforeground=TEXT,
                font=FONT_SMALL, command=self._on_mode_change,
        ...
        for txt, cmd, acc in [
            ...
            button(p, txt, cmd, accent=acc).pack(fill="x", padx=10, pady=2)

        # Opening preset
        separator(p)
        heading(p, "🎯 STARTING OPENING", fg=ACCENT).pack(fill="x", padx=10)
        self._preset_lbl = tk.Label(
            p, text="♟ Normal start", bg=PANEL_BG, fg="#666",
            font=FONT_TINY, anchor="w", wraplength=230, justify="left")
        self._preset_lbl.pack(fill="x", padx=10, pady=(2, 4))

        # Material balance
        separator(p)
        label(p, "Material balance", 8, fg="#888").pack(fill="x", padx=10)
        self.mat_lbl = tk.Label(
            p, text="=", bg=PANEL_BG, fg=TEXT,
            font=FONT_MONO_SM, anchor="center")
        self.mat_lbl.pack(fill="x", padx=10)

        # Opening book
        separator(p)
        self.book_lbl = tk.Label(
            p, text="", bg=PANEL_BG, fg="#00BFFF",
            font=FONT_TINY, anchor="w", wraplength=230)
        self.book_lbl.pack(fill="x", padx=10)
        button(p, "📂  Load Openings CSV", self._browse_csv, small=True).pack(
            fill="x", padx=10, pady=2)

        # Analyzer
        separator(p)
        heading(p, "🔍 ANALYZER (Stockfish)", fg=ACCENT).pack(fill="x", padx=10)
        self.analyzer_lbl = tk.Label(
            p, text="⚠ No analyzer", bg=PANEL_BG, fg="#FF8800",
            font=FONT_TINY, anchor="w", wraplength=230)
        self.analyzer_lbl.pack(fill="x", padx=10)
        button(p, "📂  Load Analyzer", self._browse_analyzer, small=True).pack(
            fill="x", padx=10, pady=(2, 12))
    ...
    def _build_config_ui(self):
        ...
        if self.play_mode.get() == "engine_vs_engine":
            for col, path_var, name_var, eval_var, dep_var, tag, color in [
                ...
                pv = getattr(self, path_var)
                nv = getattr(self, name_var)
                ev = getattr(self, eval_var)
                dv = getattr(self, dep_var)

                label(self.config_frame, f"◈ {col}", 9, bold=True, fg=color).pack(
                    fill="x", pady=(10, 2))
                rf = tk.Frame(self.config_frame, bg=PANEL_BG)
                rf.pack(fill="x", pady=2)
                entry(rf, pv).pack(side="left", fill="x", expand=True, ipady=4)
                button(rf, "…", lambda t=tag: self._browse(t), small=True).pack(
                    side="right", padx=(4, 0))
                entry(self.config_frame, nv, fg=color).pack(fill="x", pady=2, ipady=3)

                ef = tk.Frame(self.config_frame, bg=INFO_BG)
                ef.pack(fill="x", pady=2)
                label(ef, "Eval:", 8, bg=INFO_BG, fg="#888").pack(side="left", padx=4)
                tk.Label(ef, textvariable=ev, bg=INFO_BG, fg="#7FFF00",
                         font=FONT_MONO_XS).pack(side="left")
                label(ef, " D:", 8, bg=INFO_BG, fg="#888").pack(side="left")
                tk.Label(ef, textvariable=dv, bg=INFO_BG, fg="#7FFF00",
                         font=FONT_MONO_XS).pack(side="left")
                separator(self.config_frame, pad_y=4)
        else:
            # Human vs Engine config
            label(self.config_frame, "◈ PLAYER INFO", 9, bold=True, fg="#00FF00").pack(
                fill="x", pady=(10, 2))
            tk.Label(self.config_frame, text="Your Name:", bg=PANEL_BG, fg=TEXT,
                     font=FONT_TINY).pack(anchor="w", pady=(4, 2))
            entry(self.config_frame, self.player_name).pack(fill="x", pady=2, ipady=3)
            ...
            tk.Radiobutton(
                color_frame, text="⚪ White", variable=self.player_color, value="white",
                bg=PANEL_BG, fg="#FFD700", selectcolor=BTN_BG,
                font=(FONT_FAMILY, 9, "bold")).pack(side="left", padx=(0, 10))
            tk.Radiobutton(
                color_frame, text="⚫ Black", variable=self.player_color, value="black",
                bg=PANEL_BG, fg="#C8C8C8", selectcolor=BTN_BG,
                font=(FONT_FAMILY, 9, "bold")).pack(side="left")

            separator(self.config_frame, pad_y=8)
            heading(self.config_frame, "◈ OPPONENT ENGINE", fg=ACCENT).pack(fill="x", pady=(4, 2))
            rf = tk.Frame(self.config_frame, bg=PANEL_BG)
            rf.pack(fill="x", pady=2)
            entry(rf, self.e2_path).pack(side="left", fill="x", expand=True, ipady=4)
            button(rf, "…", self._browse_opponent, small=True).pack(side="right", padx=(4, 0))
            entry(self.config_frame, self.e2_name, fg=ACCENT).pack(fill="x", pady=2, ipady=3)
    ...
    def _build_center(self, p):
        ...
        self._black_name_lbl = tk.Label(
            self.black_banner, textvariable=self.e1_name,
            bg="#1a1a2a", fg="#C8C8C8",
            font=(FONT_FAMILY, 11, "bold"), anchor="center", pady=5)
        self._black_name_lbl.pack(side="left", expand=True)
        self._black_rank_lbl = tk.Label(
            self.black_banner, text="", bg="#1a1a2a", fg="#C8C8C8",
            font=FONT_SMALL, anchor="e", padx=8)
        self._black_rank_lbl.pack(side="right")

        # Row 3: Board area (eval bar + board + coordinates) — EXPANDS
        board_row = tk.Frame(p, bg=BG)
        board_row.grid(row=3, column=0, sticky="nsew")
        p.rowconfigure(3, weight=1)

        # Eval bar column
        eval_bar_frame = tk.Frame(board_row, bg=BG)
        eval_bar_frame.pack(side="left", fill="y", padx=(0, 4))
        tk.Label(eval_bar_frame, text="♚", bg=BG, fg="#AAA",
                 font=(FONT_FAMILY, 9)).pack(side="top")
        self.eval_canvas = tk.Canvas(
            eval_bar_frame, width=22, bg="#2A2A2A", bd=0,
            highlightthickness=1, highlightbackground="#555")
        self.eval_canvas.pack(side="top", fill="y", expand=True)
        ...
        board_inner.pack(side="left", fill="both", expand=True)
        ...
        for i in range(8):
            l = tk.Label(rf, text="", bg=BG, fg="#777",
                         font=(FONT_MONO, 9), width=2, anchor="e")
            l.pack(side="top", expand=True, fill="y")
        ...
        self.canvas = tk.Canvas(
            bcol, width=sz * 8, height=sz * 8, bg=BG, bd=0,
        ...
        self._white_name_lbl = tk.Label(
            self.white_banner, textvariable=self.e2_name,
            bg="#1c2a1c", fg="#FFD700",
            font=(FONT_FAMILY, 11, "bold"), anchor="center", pady=5)
        self._white_name_lbl.pack(side="left", expand=True)
        self._white_rank_lbl = tk.Label(
            self.white_banner, text="", bg="#1c2a1c", fg="#FFD700",
            font=FONT_SMALL, anchor="e", padx=8)
        self._white_rank_lbl.pack(side="right")

        # Row 5: Check label
        self.check_lbl = tk.Label(
            p, text="", bg=BG, fg=CHECK_SQ,
            font=(FONT_FAMILY, 10, "bold"), anchor="center")
        self.check_lbl.grid(row=5, column=0, sticky="ew", pady=(4, 0))

        # Row 6: Quality label
        self.quality_lbl = tk.Label(
            p, text="", bg=BG, fg=TEXT,
            font=(FONT_FAMILY, 13, "bold"), anchor="center")
        self.quality_lbl.grid(row=6, column=0, sticky="ew", pady=(2, 0))

        self._update_coords()

    def _on_board_resize(self, event):
        """Dynamically resize the board when the canvas changes size."""
        available = min(event.width, event.height)
        new_sq = max(40, available // 8)
        if new_sq != self.sq_size:
            self.sq_size = new_sq
            self._draw_board()
    ...
    def _build_right(self, p):
        tk.Label(p, text="GAME LOG", bg=PANEL_BG, fg=ACCENT,
                 font=(FONT_FAMILY, 12, "bold")).pack(pady=(12, 4))
        accent_line(p)

        # Move log
        label(p, "Moves (SAN):", 9, bold=True).pack(fill="x", padx=10, pady=(8, 2))
        ...
        self.move_text = scrolledtext.ScrolledText(
            mf, bg=LOG_BG, fg="#DDD", font=FONT_MONO_SM, state="disabled",
        ...
        self.move_text.tag_config("white",   foreground="#FFD700",
                                   font=(FONT_MONO, 9, "bold"))
        self.move_text.tag_config("chk",     foreground="#FF8800",
                                   font=(FONT_MONO, 9, "bold"))
        self.move_text.tag_config("result",  foreground=ACCENT,
                                   font=(FONT_MONO, 10, "bold"))
        self.move_text.tag_config("opening", foreground="#00BFFF",
                                   font=(FONT_MONO, 9, "italic"))

        separator(p)

        # Engine output log
        label(p, "Engine Output:", 9, bold=True).pack(fill="x", padx=10, pady=(4, 2))
        ...
        self.eng_log = scrolledtext.ScrolledText(
            ef, bg=LOG_BG, fg="#0FA", font=FONT_MONO_XS, state="disabled",
            relief="flat", padx=6, pady=6, insertbackground=TEXT,
        ...
        self.info_lbl = tk.Label(
            p, text="", bg=PANEL_BG, fg="#666",
            font=FONT_TINY, anchor="w", wraplength=255, justify="left")
    ...
    def _update_coords(self):
        ...
        for i, l in enumerate(self.rank_labels):
            l.config(text=str(ranks[i]))
        for i, l in enumerate(self.file_labels):
            l.config(text=files[i])
        ...
        for row in range(8):
            for col in range(8):
                ...
                if self.selected_square and (br, bc) == self.selected_square:
                    ...
                    color = LAST_TO
                if chk_king and (br, bc) == chk_king:
                    color = CHECK_SQ
                ...
                if pc and pc != ".":
                    ...
                    self.canvas.create_text(cx + 1, cy + 2, text=sym,
                                            font=(FONT_FAMILY, fsz), fill=sh)
                    self.canvas.create_text(cx, cy, text=sym,
                                            font=(FONT_FAMILY, fsz), fill=fg)
    ...
    def _log_eng(self, txt, side="W"):
        ...
        if lines > 700:
            self.eng_log.delete("1.0", "150.0")
        self.eng_log.see("end")
        self.eng_log.config(state="disabled")
    ...
    def _show_eval(self, engine, side):
        ...
        if side == "b":
            self.e1_eval.set(ev)
            self.e1_depth.set(ds)
        else:
            self.e2_eval.set(ev)
            self.e2_depth.set(ds)
        if sc is not None:
    ...
    def _browse(self, which):
        ...
        if which == 1:
            ...
            self.e1_name.set(f"{name} (Black)")
        else:
            self.e2_path.set(path)
            self.e2_name.set(f"{name} (White)")
        ...
        while self.game_running:
            ...
            if not engine or not engine.alive:
                ...
                self._end_game(
                    ...
                    f"{name}'s engine process died", wn)
                return
    ...
    def _end_game(self, result, reason, winner_name=None):
        ...
        pgn = build_pgn(
            white_name, black_name, self.board.move_history, result,
    ...
    def _start_game(self):
        if self.game_running:
            messagebox.showinfo("Running", "Stop the current game first.")
            return
        ...
        for v in (self.e1_eval, self.e2_eval, self.e1_depth, self.e2_depth):
            v.set("—")
    ...
    def _export_pgn(self):
        if not self.board.move_history:
            messagebox.showinfo("PGN", "No moves to export yet.")
            return
        pgn = build_pgn(
            self.e2_name.get(), self.e1_name.get(),
    ...
    def _show_game_over_dialog(self, result, reason, winner_name):
        ...
        w, h = 480, 660
        dialog.geometry(
            f"{w}x{h}+{(dialog.winfo_screenwidth() - w) // 2}"
        ...
        tk.Label(main_frame, text=icon, bg=BG,
                 font=(FONT_FAMILY, 48)).pack(pady=(10, 5))
        ...
        if winner_name:
            ...
            if winner_elo:
                tk.Label(main_frame, text=f"Elo: {winner_elo}  ·  {tier_lbl}",
                         bg=BG, fg=tier_col, font=FONT_HEADING).pack(pady=2)
        ...
        tk.Label(main_frame, text=reason, bg=BG, fg="#888",
                 font=FONT_HEADING).pack(pady=2)

        if self.current_opening_name:
            tk.Label(main_frame, text=f"📖  {self.current_opening_name}",
                     bg=BG, fg="#00BFFF",
                     font=(FONT_FAMILY, 10, "italic")).pack(pady=4)
        ...
        if self._move_qualities:
            ...
            summary = "  ".join(
                ...
                if q not in ("Good",))
            if summary:
                tk.Label(main_frame, text=f"Move quality: {summary}",
                         bg=BG, fg="#AAA", font=FONT_SMALL).pack(pady=2)
        ...
        for text, cmd, accent in [
            ...
            btn = tk.Button(
                btn_frame, text=text, command=cmd, bg=bg_c, fg=TEXT,
                activebackground=BTN_HOV, activeforeground="white",
                relief="flat",
